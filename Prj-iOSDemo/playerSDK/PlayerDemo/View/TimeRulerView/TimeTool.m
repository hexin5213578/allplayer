...
+(TimeMarkModel*)getPrimaryModelfromsTime:(NSString*)startTime toeTime:(NSString*)endTime{
    ...
    NSDate *labDate = [TimeTool date:startDate ToNearestMinutes:20 isStart:YES];
    ...
    NSDate *farlabDate = [TimeTool  date:endDate ToNearestMinutes:5 isStart:NO];
...
+ (NSDate *)date:(NSDate*)date ToNearestMinutes:(NSInteger)minutes isStart:(BOOL)start  {
    ...
    // Extract components.
    NSDateComponents *time = [[NSCalendar currentCalendar] components:unitFlags fromDate:date];
    ...
    if (start) {
        if (remain<dividor){
            ...
            newDate = [date dateByAddingTimeInterval:-60*(remain)];
        }else{
            // Add the remainder of time to the date to round it up evenly
            newDate = [date dateByAddingTimeInterval:60*(minutes-remain) - (60*10)];
        }
    }else{
        newDate = [date dateByAddingTimeInterval:60*(minutes-remain)];
    }
