...
-(void)initRulerViewWith:(NSMutableArray*)videArr withPraModel:(TimeMarkModel*)mark{
    NSMutableArray* markArr = [[NSMutableArray alloc]init];
    for (RecordList *model in videArr) {
        TimeMarkModel* remarkmodel = [[TimeMarkModel alloc]init];
        remarkmodel.markColor =  UIColorFromHex(0xFF669900);
        remarkmodel.startTime = model.beginTime;
        remarkmodel.endTime = model.endTime;
        remarkmodel.startSec = [TimeTool getIntervalfTime: model.beginTime since:mark.nearLabTime];
        remarkmodel.endSec = [TimeTool getIntervalfTime: model.endTime since:mark.nearLabTime];
...
-(void)updateRulerViewWithAlarmArr:(NSMutableArray*)alarmArr{
    NSMutableArray* markArr = [[NSMutableArray alloc]init];
    for (AlarmList* alarmmode in alarmArr) {
        TimeMarkModel* remarkmodel = [[TimeMarkModel alloc]init];
        remarkmodel.markColor =  [UIColor redColor];
        remarkmodel.startTime =  [TimeTool getTime:alarmmode.alarmTime withValue:-30]; //向前30s
        remarkmodel.endTime =  [TimeTool getTime:alarmmode.alarmTime withValue:30]; //向后30s
...
-(void)initRulerViewCarmeraArr:(NSArray*)arr fromStartTime:(NSString*)startTime toEndTime:(NSString*)endTime withForm:(NSString*)form {
    ...
    Payload* cameraModel = [arr objectAtIndex:self.currentCameraIndex];
    self.rulerPlayType = RulerPlayType_RoundCamera;
    __weak typeof(self) weakSelf = self;
    [self.playVideoManager getVideoArrWithCameraId:cameraModel.deviceId StartTime:startTime EndTime:endTime withForm:form finishBlock:^(NSMutableArray * _Nonnull videoArr,NSError * error) {
        if (videoArr.count > 0) {
...
-(void)findPlayRecord:(RecordList *)recordmodel PlayUrlfinishBlock:(void(^)(RecordList *model,NSString* playurl))finishBlock{
...
-(void)findPlayVideoArrfinishBlock:(void(^)(Payload* cameraModel ,NSMutableArray* videoArr))finishBlock{
    if (self.rulerPlayType == RulerPlayType_RoundCamera) {
        Payload* cameraModel  =  [self.cameraArr objectAtIndex:self.currentCameraIndex];
        [self.playVideoManager getVideoArrWithCameraId:cameraModel.deviceId StartTime:self.primaryModel.startTime EndTime:self.primaryModel.endTime withForm:self.form finishBlock:^(NSMutableArray * _Nonnull videoArr,NSError * error) {
...
-(void)findPlayDataWith:(NSString*)time finishBlock:(void(^)(NSInteger findIndex,CGFloat packtime,NSString* playurl))finishBlock{
    ...
    RecordList* precordModel = [[RecordList alloc]init];
    NSInteger findIndex = -1;
    for (RecordList* recordModel in self.videoArr) {
        CGFloat intervalsTime =  [TimeTool getIntervalfTime:time since:recordModel.beginTime];
        CGFloat intervaleTime =  [TimeTool getIntervalfTime:time since:recordModel.endTime];
    ...
    if (findIndex != -1) {
        ...
        if ([self.currentPlayRecord.beginTime isEqualToString:precordModel.beginTime]) {
            CGFloat packbacktime = [TimeTool getIntervalfTime:time since:self.currentPlayRecord.beginTime];
            finishBlock(findIndex,packbacktime,@"");
        }else{
          
            __weak typeof(self) weakSelf = self;
            [self findPlayRecord:precordModel PlayUrlfinishBlock:^(RecordList *model,NSString *playurl) {
                weakSelf.currentPlayRecord = model;
                CGFloat packbacktime = [TimeTool getIntervalfTime:time since:model.beginTime];
...
-(void)checkPlayData:(double)speed finishBlock:(void(^)(bool isMore,NSString* playurl,CGFloat startTime,Payload* cameraModel ))finishBlock{
    ...
    //获取播放地址
    __weak typeof(self) weakSelf = self;
    [self findPlayVideoArrfinishBlock:^(Payload* cameraModel ,NSMutableArray *videoArr) {
        weakSelf.videoArr = videoArr;
        __weak typeof(self) weakSelfa = weakSelf;
        RecordList *recordmodel = [videoArr objectAtIndex:self.currentPlayIndex];
        [weakSelf findPlayRecord:recordmodel PlayUrlfinishBlock:^(RecordList *model,NSString *playurl) {
            ...
            if (speed > 0 ) {
                ...
                CGFloat totalTime = [TimeTool getIntervalfTime:model.endTime since:model.beginTime];
...
-(void)updateRulerPlayTimeOffset:(CGFloat)time{
  //  [self.acTimeRuler updateTimeRuler:time];
    ...
    NSString* atime = [TimeTool getTime:time From:self.currentPlayRecord.beginTime];
