...
public class AllPlayVideoView extends GLSurfaceView implements GLSurfaceView.Renderer,
        SurfaceTexture.OnFrameAvailableListener, OnYUVDataListener {
   ...
   /* private String snaPath = Environment.getExternalStorageState().equalsIgnoreCase(Environment.MEDIA_MOUNTED)
    ...
    public void setOnScreenShotListener(OnScreenShotListener listener) {
        this.mOnScreenShotListener = listener;
    }
    ...
    public AllPlayVideoView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        setEGLContextClientVersion(2);//setEGLContextClientVersion()方法配置surface视图，设定好使用的OpenGL版本
        setRenderer(this);// 此条语句会创建启动 GLThread
        isGLReleased = false;
        setRenderMode(GLSurfaceView.RENDERMODE_WHEN_DIRTY);// 设置为脏模式：外部调用一次 requestRender() 就渲染一次，否则不重复渲染
    }
    ...
    private void initPlayer() {
        ...
        mWePlayer.setOnPreparedListener(new OnPreparedListener() {
            @Override
        ...
        mWePlayer.setOnStoppedListener(new OnStoppedListener() {
            @Override
        ...
        mWePlayer.setOnResetListener(new OnResetListener() {
            @Override
            public void onReset() {
        ...
        mWePlayer.setOnReleasedListener(new OnReleasedListener() {
            @Override
...
//        pauseWhenDrawOneFrame();
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        Log.e("lhf", "onDrawFrame startTime = " + timeMillis);
        ...
        if (snapTag) {
            ...
            try {
                ...
                SnapFileUtils.saveFile(bitmap, snapUrl, context);
        ...
        Log.e("lhf", "onDrawFrame endTime = " + timeMillis1 + "   -------  time =" + (timeMillis1 - timeMillis));
    }
    ...
    /**
     * @param dataSource
     * @param playMode       0为实时性优先，1为画质优先，为1的时候，必填crashFrames
     * @param crashFrames    范围是5-25帧
     * @param devicePlatType 设备的平台类型，只在录像回放的时候有用
     */
     ...
     * 暂停播放
     *
     * @param isReplay true 是录像回放的暂停播放，false是本地播放的暂停播放
     */
     ...
     * 恢复播放
     *
     * @param isReplay 如果为true则是录像回放的恢复播放，false则是本地播放的恢复播放
     */
