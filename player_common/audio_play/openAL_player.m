...
-(int)initOpenAL
{
    ...
    //self.LYPlayer = [[LYPlayer alloc] init];
    return ret;
}
...
//    printf("===_m_numqueued ========================%d\n",_m_numqueued);
    
    if (AL_STOPPED == stateVaue || AL_PAUSED == stateVaue || AL_INITIAL == stateVaue)
    {
        //如果没有数据,或数据播放完了
    ...
    while(_m_numprocessed--)
    {
        ...
        //得到已经播放的音频队列多少块
        _m_IsplayBufferSize++;
//        NSLog(@"_m_IsplayBufferSize======%lld",_m_IsplayBufferSize);
...
{
    //[self saveAudioData:[NSData dataWithBytes:data length:dataSize] name:@"3333333333333333.pcm" clearBefore:NO];
    int ret = 0;
