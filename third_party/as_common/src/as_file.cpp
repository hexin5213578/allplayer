...
struct dirent *readdir(DIR *d) {
    ...
    WIN32_FIND_DATAA as_fileData;
    //fail or end  
    if (!FindNextFileA(hFind, &as_fileData)) {
        return nullptr;
    }
    struct dirent *dir = (struct dirent *)malloc(sizeof(struct dirent) + sizeof(as_fileData.cFileName));
    if (!dir) return dir;
    strcpy(dir->d_name, (char *)as_fileData.cFileName);
    dir->d_reclen = (uint16_t)strlen(dir->d_name);
    //check there is file or directory  
    if (as_fileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        dir->d_type = 2;
    }
    else {
        dir->d_type = 1;
...
FILE *as_file::create_file(const char *file, const char *mode) {
    ...
    while (true) 
    {
        ...
        if (_access(dir.c_str(), 0) == -1) { //access函数是查看是不是存在
            if (mkdir(dir.c_str(), 0777) == -1) {  //如果不存在就用mkdir函数来创建
                AS_LOG(AS_LOG_WARNING, "mkdir %s failed: %s.", dir.c_str(), get_as_errmsg());
                return nullptr;
            }
        }
    }
...
bool as_file::create_path(const char *file, unsigned int mod) {
    std::string path = file;
    std::string dir;
    size_t index = 1;
    while (1) {
...
//判断是否为目录
bool as_file::is_dir(const char *path) {
...
//判断是否为常规文件
bool as_file::file_exist(const char *path) {
...
//判断是否是特殊目录
bool as_file::is_special_dir(const char *path) {
...
int as_file::delete_file(const char *path) {
    DIR *dir;
    dirent *dir_info;
...
string as_file::loadFile(const char *path) {
    ...
    if (len != fread((char *)str.data(), 1, str.size(), fp)) {
        AS_LOG(AS_LOG_WARNING, "fread %s failed: %s.", path, get_as_errmsg());
    }
    fclose(fp);
    return str;
}

bool as_file::saveFile(const string &data, const char *path) {
...
string as_file::parentDir(const string &path) {
...
string as_file::absolutePath(const string &path, const string &current_path, bool can_access_parent) {
...
void as_file::scanDir(const string &path_in, const function<bool(const string &path, bool is_dir)> &cb, bool enter_subdirectory) {
    string path = path_in;
    if (path.back() == '/') {
        path.pop_back();
    }
...
uint64_t as_file::fileSize(FILE *fp, bool remain_size) {
    if (!fp) {
        return 0;
    }
...
uint64_t as_file::fileSize(const char *path) {
    if (!path) {
        return 0;
    }
