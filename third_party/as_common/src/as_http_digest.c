...
as_parse_digest(as_digest_s *dig, const char *digest_string)
{
    ...
    //*outDigest = as_crop_sentence(digest_string);
    char* digest = digest_string + 7;
    n = as_tokenize_sentence(digest, values, ARRAY_LENGTH(values));
...
int as_digest_client_parse(as_digest_t *digest, const char *digest_string)
{
    ...
    /* Set default values */
    dig->nc = 1;
    dig->cnonce = (unsigned int)time(NULL);
...
int as_digest_server_parse(as_digest_t *digest, const char *digest_string, char** outDigest)
{
    as_digest_s *dig = (as_digest_s *) digest;
    return as_parse_digest(dig, digest_string);
}
