...
class MJPEGTrack : public Track
{
public:
	MJPEGTrack(mk_client_connection* conn, int st) : Track(conn, st) {
        m_needParsing = 0;
        m_conn->handle_connection_status(MR_CLIENT_MEDIA_ATTRIBUTE);
    }
    virtual ~MJPEGTrack() = default;
...
private:
	int32_t handleOneFrame(RTP_PACK_QUEUE& rtpFrameList);

    uint8_t     qtables_[128][128];
    uint8_t     qtables_len_[128];
